           ...
           foreach ($_GET as $key => $v) {
            ...
            foreach ($cadenas_no as $key => $v) {
                ...
                $pos = strpos($sql, $buscar);                 
                 if ($pos !== false) {
                   return array("error" => true, "msg" => "Acceso denegado (3) !!!!");
                 }
            }
          }
    }
...
{ 
    ...
    $insertar['menu'] = MENU;
    $insertar['user_agent'] = $db->escape_string($_SERVER['HTTP_USER_AGENT']);
    $insertar['refer'] = $_SERVER['HTTP_REFERER'] ;
    $insertar['ip'] = $_SERVER['REMOTE_ADDR'];
    $insertar['id_usuario'] = $_SESSION['persona_id'];

    $db->insert("bitacoras_sistema",$insertar);


}


function insertar_errores($codigo,$error,$data)
{ 
    global $db;
    ...
    $insertar['error'] = cifrar_texto($error);
    $insertar['data'] = serialize($data);
    $insertar['menu'] = MENU;
    $insertar['user_agent'] = $db->escape_string($_SERVER['HTTP_USER_AGENT']);
    $insertar['refer'] = $_SERVER['HTTP_REFERER'] ;
    $insertar['ip'] = $_SERVER['REMOTE_ADDR'];
    ...
    $db->insert("errores_sistema",$insertar);
...
function decifrar_texto($string)
{
   $pass = '12345';
   $method = 'aes256';
   return quitar_cosas_raras(openssl_decrypt($string, $method, $pass));
}
...
function get_colores($num){
  global $db;
  $color = @$db->select_one("SELECT nombre FROM colores WHERE id='$num'");
  if ($color) { return $color; }else{
    $n = rand(1,10);
    get_colores($num);
  }
  return false;
...
include_once("funciones_general_sidpyba.php");
?>